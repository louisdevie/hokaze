...
interface TestData<T> {
  field: AnyField<T, AnyField<T, unknown>>
  defaultBlankValue: T
  customBlankValue: T
}
...
ref(fruits)
Ref.fromKey(fruits, 0)
Ref.fromKey(fruits, 1)
...
test.each(allTestData)('the blank value of any field can be customized', (testData: TestData<unknown>) => {
  const withBlankValue = testData.field.withBlankValue(testData.customBlankValue)
...
test.each(allTestData)('any field can be made read-only', (testData: TestData<unknown>) => {
  const readOnly = testData.field.readOnly
...
test.each(allTestData)('any field can be made write-only', (testData: TestData<unknown>) => {
  const writeOnly = testData.field.writeOnly
...
test.each(allTestData)('any field can be flagged as the id', (testData: TestData<unknown>) => {
  const asId = testData.field.asKey
...
test.each(allTestData)('any field can be made optional', (testData: TestData<unknown>) => {
  const optional = testData.field.optional
...
test.each(allTestData)('any field can be made nullable', (testData: TestData<unknown>) => {
  const nullable = testData.field.nullable
